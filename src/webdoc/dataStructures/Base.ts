...
\`\`\`
    ...
    protected getPropertyEntry(prop: PropertyDoc, useSignature = false)
    {
        ...
        if (useSignature)
        {
            res += `<code>${this.replaceUnsafeStrings(this.createSignature(prop as any))}</code>\n\n`;
        }
        else
        {
            if (dataType === 'unknown')
            ...
            res += `<code>${this.replaceUnsafeStrings(dataType)}</code>\n\n`;
        }
...
[${pkg.name}](../modules/${pkg.mdName}).${path}
